...
function placeTree(x,z,h,seed){
  if(h+1>=H) return;
  const trunk=5+Math.floor(n2(x,z,seed+3)*3); // 5-7 tall
  for(let ty=1;ty<=trunk;ty++) if(h+ty<H) sb(x,h+ty,z,4);
  const cr=2+Math.floor(n2(x*1.3,z*0.9,seed+9)*1.5); // canopy radius 2-3
  for(let lx=-(cr+1);lx<=cr+1;lx++) for(let lz=-(cr+1);lz<=cr+1;lz++) for(let ly=trunk-2;ly<=trunk+cr+1;ly++){
    const dd=lx*lx*0.85+lz*lz*0.85+(ly-trunk)*(ly-trunk)*1.1;
    if(dd<=(cr+0.6)*(cr+0.6)&&h+ly>=0&&h+ly<H){
...
function buildPvpTower(CX,CZ){
  ...
  // Clear any blocks above ground in tower footprint
  for(let x=-4;x<=4;x++) for(let z=-4;z<=4;z++)
    if(Math.sqrt(x*x+z*z)<=4.5) for(let dy=0;dy<20;dy++) sb(TX+x,TY+dy,TZ+z,0);
  // Circular stone base (r=4, 2 thick)
  for(let x=-4;x<=4;x++) for(let z=-4;z<=4;z++)
    if(Math.sqrt(x*x+z*z)<=4.3){sb(TX+x,TY-1,TZ+z,3);sb(TX+x,TY,TZ+z,3);}
  // Stone column 3×3, 14 tall
  for(let dy=1;dy<=14;dy++){
    for(let x=-1;x<=1;x++) for(let z=-1;z<=1;z++) sb(TX+x,TY+dy,TZ+z,3);
    if(dy>8) sb(TX,TY+dy,TZ,0); // hollow top for depth
  }
  // Circular observation deck (r=4, 2 thick)
  const DK=TY+15;
  for(let x=-4;x<=4;x++) for(let z=-4;z<=4;z++){
    if(Math.sqrt(x*x+z*z)<=4.3){sb(TX+x,DK,TZ+z,3);sb(TX+x,DK+1,TZ+z,3);}
  }
  // Wood railing on deck edge
  for(let x=-4;x<=4;x++) for(let z=-4;z<=4;z++){
    const d=Math.sqrt(x*x+z*z);
    if(d>=3.3&&d<=4.3){sb(TX+x,DK+2,TZ+z,4);sb(TX+x,DK+3,TZ+z,4);}
  }
  // 4 corner wood pillars (visible support for dome)
  [[-3,-3],[-3,3],[3,-3],[3,3]].forEach(([ox,oz])=>{
    for(let dy=2;dy<=11;dy++) sb(TX+ox,DK+dy,TZ+oz,4);
  });
  // Sphere dome (r=5, hollow shell of wood)
  const DR=5;
  for(let dy=0;dy<=DR*2;dy++){
    const cy_r=dy-DR;
    const r=Math.sqrt(Math.max(0,DR*DR-cy_r*cy_r));
    for(let dx=-Math.ceil(r)-1;dx<=Math.ceil(r)+1;dx++) for(let dz=-Math.ceil(r)-1;dz<=Math.ceil(r)+1;dz++){
      const d=Math.sqrt(dx*dx+dz*dz);
      if(d<=r+0.55&&d>=r-1.0){const wy=DK+4+dy;if(wy<H)sb(TX+dx,wy,TZ+dz,4);}
    }
  }
  // Spire at top
  for(let dy=0;dy<=3;dy++) sb(TX,DK+4+DR*2+dy,TZ,4);
}

function buildPvpRuins(CX,CZ,seed){
  // Large step pyramids at cardinal directions (r=11)
  [0,90,180,270].forEach(deg=>{
    const rad=deg*Math.PI/180;
    const rx=Math.floor(CX+Math.cos(rad)*11),rz=Math.floor(CZ+Math.sin(rad)*11);
    const gy=pvpGroundY(rx,rz);if(gy<=1)return;
    for(let step=0;step<4;step++){const sr=4-step;
      for(let x=-sr;x<=sr;x++) for(let z=-sr;z<=sr;z++) if(gy+step+1<H) sb(rx+x,gy+step+1,rz+z,3);}
    // Debris / crumble blocks around each ruin
    for(let i=0;i<12;i++){
      const ang=n2(i*1.3,seed+deg,i+7)*Math.PI*2,r=3+n2(i,seed+deg+1,i*2+3)*5;
      const bx=Math.floor(rx+Math.cos(ang)*r),bz=Math.floor(rz+Math.sin(ang)*r);
      if(bx>=1&&bx<W-1&&bz>=1&&bz<D-1){const gy2=pvpGroundY(bx,bz);if(gy2>0)sb(bx,gy2+1,bz,3);}
    }
  });
  // Medium ruins at diagonals (r=14)
  [45,135,225,315].forEach(deg=>{
    const rad=deg*Math.PI/180;
    const rx=Math.floor(CX+Math.cos(rad)*14),rz=Math.floor(CZ+Math.sin(rad)*14);
    const gy=pvpGroundY(rx,rz);if(gy<=1)return;
    for(let step=0;step<3;step++){const sr=3-step;
      for(let x=-sr;x<=sr;x++) for(let z=-sr;z<=sr;z++) if(gy+step+1<H) sb(rx+x,gy+step+1,rz+z,3);}
    for(let i=0;i<8;i++){
      const ang=n2(i*1.7,seed+deg+2,i+5)*Math.PI*2,r=2+n2(i,seed+deg+3,i*3+2)*4;
      const bx=Math.floor(rx+Math.cos(ang)*r),bz=Math.floor(rz+Math.sin(ang)*r);
      if(bx>=1&&bx<W-1&&bz>=1&&bz<D-1){const gy2=pvpGroundY(bx,bz);if(gy2>0)sb(bx,gy2+1,bz,3);}
    }
  });
  // Small stone outcrops scattered at mid-ring (r=6-8)
  for(let i=0;i<8;i++){
    const deg=i*45+22;const rad=deg*Math.PI/180;
    const r=6+n2(i,seed+99,i+4)*2.5;
    const rx=Math.floor(CX+Math.cos(rad)*r),rz=Math.floor(CZ+Math.sin(rad)*r);
    if(rx<1||rx>=W-1||rz<1||rz>=D-1)continue;
    const gy=pvpGroundY(rx,rz);if(gy<=1)continue;
    for(let x=-1;x<=1;x++) for(let z=-1;z<=1;z++) sb(rx+x,gy+1,rz+z,3);
    sb(rx,gy+2,rz,3);
  }
}
...
function generatePvpIsland(seed){
  ...
  // ── 1. Terrain base — NO trees yet ──────────────────
  for(let x=0;x<W;x++) for(let z=0;z<D;z++){
    const dx=x-CX,dz=z-CZ,dist=Math.sqrt(dx*dx+dz*dz);
    if(dist>=R) continue;
    const roll=sn(x,z,8,seed)*2.5+sn(x,z,16,seed+1)*1.4+sn(x,z,4,seed+2)*0.7;
    const edgeDip=Math.max(0,(dist-18)*0.55);
    const h=Math.min(Math.floor(BASE_H+roll-edgeDip),H-18);
    const slab=Math.max(5,Math.floor((R-dist)*0.42+4));
    const hBot=Math.max(2,h-slab);
    for(let y=hBot;y<=h;y++){
      const ft=h-y,onEdge=dist>R-5;
      if(ft===0&&!onEdge) sb(x,y,z,1);       // grass
      else if(ft<=3&&!onEdge) sb(x,y,z,2);   // dirt
      else sb(x,y,z,3);                        // stone (core + edge cliffs)
    }
  }

  // ── 2. Tower (builds on terrain, clears its footprint) ─
  buildPvpTower(CX,CZ);

  // ── 3. Ruins (scattered, placed on terrain) ────────────
  buildPvpRuins(CX,CZ,seed);

  // ── 4. Trees — LAST, only on bare grass, outside tower ─
  for(let x=2;x<W-2;x++) for(let z=2;z<D-2;z++){
    const dx=x-CX,dz=z-CZ,dist=Math.sqrt(dx*dx+dz*dz);
    if(dist<10||dist>20) continue;   // ring between tower and cliff
    const gy=pvpGroundY(x,z);
    if(gb(x,gy,z)!==1) continue;     // only grass surface
    if(gb(x,gy+1,z)!==0) continue;   // must have air above (no ruin block)
    const noise=n2(x*3.1,z*2.8,seed+5)+sn(x,z,6,seed+6)*0.35;
    if(noise>0.89) placeTree(x,z,gy,seed); // sparse
  }

  // ── 5. Chests ───────────────────────────────────────────
  [0,45,90,135,180,225,270,315].forEach(deg=>{
    const rad=deg*Math.PI/180;
    const cx2=Math.floor(CX+Math.cos(rad)*13),cz2=Math.floor(CZ+Math.sin(rad)*13);
...
function buildWorld(){
  ...
  for(let x=0;x<W;x++) for(let y=0;y<H;y++) for(let z=0;z<D;z++){
    ...
    for(const fc of FACES){
      ...
      // Per-face noise jitter → simulates block texture detail (shader look)
      const fv=(n2(x*7.3+fc.d[0]*3.7+y*0.9,z*5.9+fc.d[2]*2.1+y*1.3,77.7)-0.5)*0.15;
      const shade=fc.bri*hshade*(1+fv);
      for(const[cx,cy,cz]of fc.c){pos.push(x+cx,y+cy,z+cz);col.push(Math.max(0,Math.min(1,bc[0]*shade)),Math.max(0,Math.min(1,bc[1]*shade)),Math.max(0,Math.min(1,bc[2]*shade)));nor.push(fc.d[0],fc.d[1],fc.d[2]);}
